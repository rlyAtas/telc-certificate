...
src/
  ...
  config.ts                      # централизованные константы приложения
  db.ts                          # prisma client
  i18n/
  ...
  services/
    ...
    subscribeRateLimitService.ts # in-memory лимит запросов POST /subscribe по IP
    dbBackupService.ts           # создание и удаление snapshot SQLite
    alertService.ts              # отправка технических алертов в Telegram
...
- `TELEGRAM_BOT_TOKEN` — токен Telegram-бота для отправки алертов;
- `TELEGRAM_CHAT_ID` — chat id, куда отправлять алерты;
...
- `POST /subscribe` — защита от ботов (honeypot + проверка времени заполнения), лимит по IP, валидация, проверка открытой заявки по e-mail, создание записи и отправка e-mail с токеном подтверждения;
...
- язык интерфейса: `language` (сохраняется в БД и используется для страниц/писем этой заявки);
...
- проверяется окно поиска: `45` дней относительно `examDate`;
- при полном проходе окна поиск откладывается на `4` часа;
- при сетевой ошибке внешнего API повтор через `30` секунд;
...
### 9.3 Telegram Bot API (оповещения)
- используется в `src/services/alertService.ts`;
- отправка выполняется методом `sendMessage`;
- алерт отправляется при успешном старте сервера (тихое уведомление);
- алерт отправляется при падении на старте сервера (обычное уведомление);
- таймаут HTTP-запроса к Telegram — `5` секунд;
- сервис алертов не должен ронять приложение: при ошибке отправки возвращается `false`, ошибка только логируется.

## 10. Логирование и наблюдаемость
- единый логгер: `src/services/logger.ts` (`pino`);
- в `development` включён `pino-pretty`;
- логирование в прикладных модулях выполняется через `logger` в строковом формате с префиксами;
- для `POST /subscribe` используется in-memory лимитер по IP (`3` запроса за `60` секунд);
- текущий лимитер хранится в памяти процесса: счётчики сбрасываются при рестарте и не синхронизируются между несколькими инстансами приложения.
...
4. Нет автотестов (ни unit, ни интеграционных), поэтому регрессии возможны даже при небольших изменениях.
5. Cron обрабатывает только одну запись за тик, что ограничивает пропускную способность при росте нагрузки.
...
4. Ввести ротацию/ретенцию логов и правила алертинга по уровням `warn/error`.
